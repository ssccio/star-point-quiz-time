...
export const useSupabaseSubscription = (
  ...
  const { onConnectionLost, onReconnected, enableToasts = false, debugLabel = 'Supabase' } = options;
    ...
    try {
      ...
      if (subscription && subscription.subscribe) {
        ...
        subscription.subscribe((status: string) => {
          ...
          console.log(`${debugLabel}: Subscription status changed:`, prevStatus, '->', status);
          ...
          if (status === 'SUBSCRIBED') {
            ...
            console.log(`${debugLabel}: Connection lost, will attempt reconnection`);
            if (enableToasts) {
  ...
  }, [createSubscriptionFn, onConnectionLost, onReconnected, enableToasts, debugLabel, ...deps]);

  // Handle visibility changes (phone lock/unlock)
  const handleVisibilityChange = useCallback(() => {
    if (document.visibilityState === 'visible') {
      console.log(`${debugLabel}: App became visible - checking connection health`);
